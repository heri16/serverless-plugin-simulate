...
const getFunctionConfig = (serverless, functionName, functionConfig) => {
  const distPath = (
    serverless.service.custom.simulate.dist ? `/${serverless.service.custom.simulate.dist}` : ''
  )
...
const getAppsyncMappings = (serverless) => {
  ...
  return mappingTemplates.map((tpl) => ({
    ...
    requestMappingTemplate: fs.readFileSync(
      `${mappingTemplatesLocation}/${tpl.request}`,
      'utf8'
    ),
    typeName: tpl.type,  // "Query"
    responseMappingTemplate: fs.readFileSync(
      `${mappingTemplatesLocation}/${tpl.response}`,
      'utf8'
    ),
  }))
}
  ...
  const graphqlFunc = (lambdaRunner) => (req) => {
    ...
    for (const datasource of datasources.filter(ds => ds.lambdaFunctionConfig)) {
      ...
      for (const mp of mappings.filter(m => m.dataSourceName === datasource.name)) {
        ...
        context[mp.typeName][mp.fieldName] = new LambdaMapper({ connector, mapping: mp })
      }
    }
