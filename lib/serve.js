...
// Optional require('apollo-server-express')
let graphqlExpress
let graphiqlExpress
try {
...
const startWithAppsync = (graphqlFunc, endpoints, corsMethodsForPath, port, lambdaPort, logger) => {
  const log = logger || noOp
  // When simulating API Gateway use remote lambda if available otherwise local
  ...
  server.use('/graphql', bodyParser.json(), graphqlExpress(graphqlFunc(lambda)))

  // GraphiQL, a visual editor for queries
  server.use('/graphiql', graphiqlExpress({ endpointURL: '/graphql' }))

  // Add API Gateway endpoints to `req.context`
  ...
  log(`GraphQL API: http://localhost:${port}/graphql`)
  log(`GraphQL Interactive Editor: http://localhost:${port}/graphiql`)
